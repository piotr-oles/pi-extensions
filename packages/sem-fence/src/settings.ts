 ...
 *   1. --sem-fence-block CLI flag
 *   2. semFence.mode in .pi/settings.json  (project)
 *   3. semFence.mode in ~/.pi/agent/settings.json  (global)
 *   4. "warn"  (built-in default)
 */
export async function loadMode(pi: ExtensionAPI, cwd: string): Promise<FenceMode> {
  if (pi.getFlag("sem-fence-block") === true) return "block";
