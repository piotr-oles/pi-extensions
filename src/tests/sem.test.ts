...
const IMPACT_RESULT: SemImpactResult = {
  entity: { entityId: "src/utils.ts::function::myFunc", name: "myFunc", type: "function", file: "src/utils.ts", lines: [1, 5] },
  dependencies: [],
  dependents: [],
...
describe("semImpact", () => {
  it("calls sem impact with --json and entity name", async () => {
    ...
    expect(exec).toHaveBeenCalledWith("sem", expect.arrayContaining(["impact", "--json", "myFunc"]), expect.anything());
  });
...
describe("semDiff", () => {
  ...
  it("uses json format when explicitly requested", async () => {
    ...
    expect(exec).toHaveBeenCalledWith("sem", expect.arrayContaining(["--format", "json"]), expect.anything());
  });
