...
### `pi-steer` (`packages/pi-steer`)
Injects selected instruction sections into the system prompt. Bundled sections include YAGNI ("You Aren't Gonna Need It") discipline. Selection is controlled by the `pi-steer` flag or `PI_STEER` environment variable. Prefix a section with `-` to exclude it from the default set. Default: all sections. Selection and composed text are stored in the session and restored on resume.
