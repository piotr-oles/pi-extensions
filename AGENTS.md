...
Shows the current branch's GitHub PR in the footer via `ctx.ui.setStatus("pi-pr", …)`: a lifecycle letter (`D` draft, `O` open, `M` merged, `C` closed) and a clickable `#n` link (OSC 8 hyperlink to the PR URL, both colored by lifecycle — closed is red, merged uses a raw 256-color purple tuned to the terminal background via `COLORFGBG`, brighter on dark, since no theme token is purple). For active PRs (draft/open) it also shows a `●` CI-rollup glyph (always shown, colored by state, `none` dim so its position stays stable) and two conditional glyphs in fixed order: a red `‼` conflict glyph only when the PR has merge conflicts, and a review verdict only when decided (`✓` green approved, `✗` red changes-requested). Merged and closed PRs are terminal states: they drop the conflict and review glyphs and show the CI glyph only when CI failed. Polls on a timer and also fires a one-off refresh when a `tool_result` shows the agent ran `gh pr create`, so a newly opened PR shows up immediately.
