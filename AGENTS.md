...
Shows the current branch's GitHub PR in the footer via `ctx.ui.setStatus("pi-pr", …)`: a lifecycle glyph (`◇` draft, `◆` open, `◈` merged) and a clickable `#n` link (OSC 8 hyperlink to the PR URL, both colored by lifecycle; closed PRs render nothing) plus a `●` CI-rollup glyph (always shown, colored by state, `none` dim so its position stays stable) and two conditional glyphs in fixed order: a red `‼` conflict glyph only when the PR has merge conflicts, and a review verdict only when decided (`✓` green approved, `✗` red changes-requested). Polls on a timer and also fires a one-off refresh when a `tool_result` shows the agent ran `gh pr create`, so a newly opened PR shows up immediately.
